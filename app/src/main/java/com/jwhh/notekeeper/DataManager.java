...
import static com.jwhh.notekeeper.NoteKeeperDatabaseContract.CourseInfoEntry.COLUMN_COURSE_TITLE;
...
/*            ourInstance.initializeCourses();
	...
	public static void LoadFromDatabase(NoteKeeperOpenHelper noteKeeperOpenHelper) {
		...
		String[] coursesColumn = {CourseInfoEntry.COLUMN_COURSE_ID, COLUMN_COURSE_TITLE};
		//THE LAST ONE IS THE SORT ... IT RETERS AND SORT BY THE TITLE USE COLUMN_COURSE_TITLE+" DESC" for desending order
		Cursor courseCursor = db.query(CourseInfoEntry.TABLE_NAME, coursesColumn, null, null, null, null, COLUMN_COURSE_TITLE+" DESC");
		loadCoursesFromDatabase(courseCursor);
		String[] noteColumn = {COLUMN_NOTE_TITLE, COLUMN_NOTE_TEXT, NoteInfoEntry.COLUMN_COURSE_ID,NoteInfoEntry._ID};
		...
		//cursor give us data by row by intiial it is a row behind the 1st row

	}

	private static void loadCoursesFromDatabase(Cursor cursor) {
		int courseIdPos = cursor.getColumnIndex(CourseInfoEntry.COLUMN_COURSE_ID);
		int coursTitlePos = cursor.getColumnIndex(COLUMN_COURSE_TITLE);

		DataManager dm = DataManager.getInstance();
		dm.mCourses.clear();
		while (cursor.moveToNext()) {
			String courseId = cursor.getString(courseIdPos);
			String courseTitle = cursor.getString(coursTitlePos);
	...
	public int createNewNote(CourseInfo course, String noteTitle, String noteText) {
		int newNote = createNewNote();
		NoteInfo noteInfo = getNotes().get(newNote);
